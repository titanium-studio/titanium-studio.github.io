...
const { each } = tools, { add, remove, contains, device } = DOM, { box_height, box, card, btn, link, Div, $event } = Box, { _nav_, _back_, _body_ } = gVars, main = Main.init();
...
fetch("/src/json/index.json")
  ...
  .then(z => {
    ...
    // DSGN
    each(imgs, (x, i) => set(0, b_h(box(true, { src: core, name: x.url, format: frm }, x.p), { gridArea: "x" + i })).reverse(true))
    // WORK
